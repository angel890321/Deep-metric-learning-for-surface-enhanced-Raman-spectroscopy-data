...
class SpectralDataset(Dataset):
    ...
    def __init__(self, X_fn, y_fn,p_fn, idxs, transform, w=50, dw=25):
        if type(X_fn) == str:
            self.X = np.load(X_fn)
        else:
            self.X = X_fn
    ...
    def __getitem__(self, idx):
        ...
        x_segment = [
            x[:, i : i + self.w] for i in range(0, x.shape[1] - self.w, self.dw)
        ]
        # y_segment = [y[i:i+self.w] for i in range(0, len(y)-self.w, self.dw)]
        if self.transform:
            x_segment = self.transform(x_segment)
        return (x_segment, y, p)
...
def sliding_spectral_dataloader(
    X_fn,
    y_fn,
    p_fn,
    idxs,
...
def spectral_dataloaders(
    ...
    trainloader = sliding_spectral_dataloader(
        X_fn,
        y_fn,
        train_idxs,
        batch_size=batch_size,
    ...
    valloader = sliding_spectral_dataloader(
        X_fn,
        y_fn,
        val_idxs,
        batch_size=batch_size,
    ...
    testloader = sliding_spectral_dataloader(
        X_fn,
        y_fn,
        test_idxs,
        batch_size=batch_size,
